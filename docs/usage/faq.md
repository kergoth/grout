...
> 3. **Protocol** - Try switching between HTTP and HTTPS. If you get an "Access Forbidden" error, you're likely
>    using the wrong one.
...
> ROM filenames in your RomM library for the same platform, with a forgiving case-insensitive fallback. For the
> best experience, keep your local ROM filenames consistent with the names in your RomM library.
>
...
> (or `L2` on Miyoo devices). From there Grout will list the BIOS files along with their status (Ready or Missing).
...
> **What is Kid Mode?**
>
> Kid Mode hides Settings, Save Sync, Game Options, and BIOS downloads, leaving only game browsing and downloading.
> To temporarily access these while Kid Mode is enabled, press `L1 + R1 + Menu` during the Grout splash screen.
